# Anak Agung Gde Weida Ksatriawarma

Hi there! 👋 I'm Weida, welcome to my GitHub profile! With AI assistant I almost can have more things to do! Bitcoin will $1M!

## 🚀 About Me

- 🌱 I’m currently learning about web development, and artificial intelligence. Will focus on that 2 things.
- 💼 I'm offering services in website creation for company profiles, personal portfolios, and more.
- 📚 I’m majoring in IT with courses including Web Development, Computer Networks, Microcontroller and Internet of Things and more, still learning more!

## 🔧 More Confident With This Technologies & Tools
[![My Skills](https://skillicons.dev/icons?i=html,css,js,py,wordpress,bootstrap,linux,ubuntu,vscode,windows,reactjs,shadcn,nextjs,supabase,vercel,tailwind)](https://skillicons.dev)

More: Elementor, Claude Code.

### Collaboration Tools
[![My Skills](https://skillicons.dev/icons?i=discord,github)](https://skillicons.dev)

## More and Things I have explore
[![My Skills](https://skillicons.dev/icons?i=arduino,bash,cpp,debian,docker,figma,kali,md,mint,mysql,nginx,nodejs,npm,postman,raspberrypi)](https://skillicons.dev)
